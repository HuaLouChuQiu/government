...
class pdoSql {
    private $dbms = "mysql";
    //private $host = DB_HOST;       //数据库主机名
    //private $dbName = "flight_line";
    //private $user = DB_USER;
    //private $pwd = DB_PASS;
    private $pdoObject;

    public function __construct($host = DB_HOST,$user = DB_USER,$pwd = DB_PASS,$dbName=DB_NAME,$charset="utf8"){
        
        try{
            $setcharset = array(PDO::MYSQL_ATTR_INIT_COMMAND=>"set names ".$charset);
            
            $dsn = $this->dbms.":host=".$host.";dbname=".$dbName;       //构造pdo dsnv
            $this->pdoObject = new PDO($dsn,$user,$pwd,$setcharset);
